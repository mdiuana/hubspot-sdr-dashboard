...
interface ClientPresentadas { name: string; agendadas: number; presentadas: number; ratio: number }
interface PresentadasData {
  total: { agendadas: number; presentadas: number; ratio: number }
  clients: ClientPresentadas[]
}
...
function rateColor(pct: number): string {
  ...
  if (pct >= 30)  return "from-amber-500 to-orange-500"
...
function RateCard({ client, index, visible }: {
  client: ClientPresentadas
  index: number
  visible: boolean
}) {
  const pct = client.agendadas > 0 ? Math.round(client.ratio * 100) : 0
  const pctAnim = useCountUp(pct, visible)
    ...
    >
      ...
      <div className="relative p-4 pb-3 flex-1 flex flex-col items-center">
        ...
        <p className="text-[11px] font-semibold text-muted-foreground/70 mt-2 tabular-nums">
          {presentadasAnim}/{agendadasAnim} <span className="font-normal">presentadas</span>
        </p>
      </div>
...
export function PresentationRate() {
  ...
  const [data, setData]           = useState<PresentadasData | null>(null)
  ...
  function doFetch() {
    ...
    fetch(`/api/stats/presentadas?${params}`)
      .then(r => r.json())
      .then(json => { if (json.success) { setData(json); setTimeout(() => setVisible(true), 40) } })
      .catch(e => { console.error(e); setVisible(true) })
  ...
  const totalPct = data?.total.agendadas
    ? Math.round(data.total.ratio * 100)
    : 0
    ...
    <div className="glass rounded-2xl p-5 space-y-4 animate-fade-in-up">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 p-2 shadow-md shadow-emerald-500/25">
            <CheckCircle2 className="h-4 w-4 text-white" />
          </div>
          <div>
            <h2 className="text-sm font-bold leading-tight">Tasa de Presentación</h2>
            <p className="text-xs text-muted-foreground">
              {loading
                ? "Cargando..."
                : `${data?.total.presentadas ?? 0}/${data?.total.agendadas ?? 0} presentadas (${totalPct}%) ${rangeLabel()}`
              }
            </p>
          </div>
        </div>
